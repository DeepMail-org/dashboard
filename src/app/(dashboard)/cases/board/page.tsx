...
function CaseCard({
	...
	return (
		<div
			...
			className="group cursor-grab rounded-xl border border-border bg-[#ffffff] dark:bg-[#1a1c20] p-3.5 shadow-sm transition-all hover:border-border-hover hover:shadow-md active:cursor-grabbing active:opacity-80 active:scale-95"
		>
			{/* Header */}
			...
			{c.tags.length > 0 && (
				<div className="mt-2 flex flex-wrap gap-1">
					{c.tags.slice(0, 2).map((tag) => (
						<span
							key={tag}
							className="rounded bg-fg/5 px-1.5 py-px text-[9px] text-dimmed"
						>
							{tag}
						</span>
					))}
...
export default function CasesBoardPage() {
	...
	const [dragOverStatus, setDragOverStatus] = useState<CaseStatus | null>(
		null,
	);
	...
	const handleDrop = async (caseId: string, targetStatus: CaseStatus) => {
		...
		setCases((prev) =>
			prev.map((x) =>
				x.id === caseId ? { ...x, status: targetStatus } : x,
			),
		);
		await updateCaseStatus(caseId, targetStatus);
		toast.success(
			`Case ${caseId} moved to ${COLUMN_CONFIG[targetStatus].label}`,
		);
	};
	...
	return (
		<PageWrapper noPadding>
			<div className="flex flex-col h-full overflow-hidden">
				...
				<div className="flex gap-4 overflow-x-auto p-6 shrink-0 border-b border-border">
					{COLUMN_ORDER.map((status) => (
						<div
							key={status}
							className="rounded-xl border border-border bg-linear-to-b from-fg/5 to-fg/1 px-4 py-3 min-w-[120px]"
						>
							<div
								className={cn(
