...
export default function SandboxWorkersPage() {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6 max-w-7xl mx-auto w-full">
        {workers.map((worker) => (
          <div key={worker.id} className="rounded-2xl border border-border bg-surface overflow-hidden shadow-card group transition-colors">
            {/* Header */}
            <div className="px-5 py-4 border-b border-border flex items-start justify-between bg-fg/2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={cn(
                      ...
                      worker.status === "busy" ? "text-yellow-400" : "text-muted"
                    )}>
                      {worker.status}
                    </span>
              ...
              <div className="flex gap-1">
                <button className="p-1.5 rounded hover:bg-fg/5 text-muted hover:text-fg transition-colors" title="Restart">
                  <RefreshCw className="h-4 w-4" />
                </button>
                <button className="p-1.5 rounded hover:bg-fg/5 text-muted hover:text-fg transition-colors" title="Drain">
                  <Archive className="h-4 w-4" />
                </button>
                <button className="p-1.5 rounded hover:bg-danger-soft text-muted hover:text-danger transition-colors" title="Destroy">
            ...
            {/* Footer Stats */}
            <div className="px-5 py-3 border-t border-border bg-fg/2 flex items-center justify-between">
