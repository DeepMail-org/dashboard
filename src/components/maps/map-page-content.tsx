...
import {
  Map, MapControls, MapClusterLayer, MapArc, MapPopup,
  MapMarker, MarkerContent, useMap, type MapArcDatum,
...
// ── Map inner component (inside Map context for useMap) ───────────────────
function MapInner({
  points,
  mode,
  clustersEnabled,
  ...
  const handlePointClick = (feature: GeoJSON.Feature<GeoJSON.Point>, coords: [number, number]) => {
    ...
    <>
      ...
      {mode === "arcs" && (
        <>
          ...
          {/* Origin markers */}
          {points.map((p) => (
            <MapMarker key={p.ip} longitude={p.lon} latitude={p.lat}>
              <MarkerContent>
                <button
                  onClick={() => {
      ...
      {mode === "2d" && (
        <MapClusterLayer
          ...
          clusterThresholds={clustersEnabled ? [5, 20] : [9999, 99999]}
    ...
    <div className="relative flex shrink-0">
      ...
      >
        <div className="border-b border-border px-4 py-3 shrink-0">
          <h2 className="text-xs font-semibold text-fg">Threat Origins</h2>
          <p className="mt-0.5 text-[10px] text-muted">
            {points.length} locations · {points.reduce((s, p) => s + p.count, 0).toLocaleString()} total threats
          </p>
        </div>
          ...
          {sorted.map((p) => (
            <button
              ...
              className={cn(
                ...
                selectedIp === p.ip && "bg-accent/8 border-l-2 border-l-accent",
              )}
            >
              <div className="min-w-0">
                ...
                <p className="font-mono text-[9px] text-dimmed truncate">{p.ip}</p>
              </div>
              <SeverityPill severity={p.severity} size="xs" showDot={false} />
    ...
    <div className="flex h-[calc(100dvh-64px)] flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border bg-surface/80 px-5 py-2.5 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-display text-sm font-semibold text-fg">Geo Threat Map</h1>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="relative flex-1">
          ...
          {/* Severity legend */}
          <div className="absolute bottom-4 left-4 z-10 flex items-center gap-3 rounded-xl border border-border bg-surface/90 px-4 py-2.5 backdrop-blur-sm shadow-card">
            ...
            <div className="flex items-center gap-1.5">
              <Target className="h-3 w-3 text-accent" />
              <span className="text-[10px] text-accent">HQ</span>
            </div>
          </div>
          ...
          <div className="absolute top-4 right-4 z-10 grid grid-cols-2 gap-2">
            {[
              { label: "Total Threats", value: points.reduce((s, p) => s + p.count, 0).toLocaleString(), color: "text-fg" },
              { label: "Critical Sources", value: points.filter((p) => p.severity === "critical").length.toString(), color: "text-danger" },
              { label: "Countries", value: new Set(points.map((p) => p.country)).size.toString(), color: "text-accent" },
              { label: "Active Arcs", value: points.length.toString(), color: "text-info" },
            ].map((s) => (
              <div key={s.label} className="rounded-lg border border-border bg-surface/90 px-3 py-2 backdrop-blur-sm shadow-card">
                <div className={cn("font-display text-lg font-bold", s.color)}>{s.value}</div>
