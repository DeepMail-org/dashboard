/**
 * ECharts theme that reads from CSS custom properties so it adapts
 * automatically when the user switches between dark/light mode.
 *
 * IMPORTANT: Call getEchartsTheme() at render time (not module-level)
 * so it picks up the current CSS variable values.
 */
...
export function getEchartsTheme() {
  ...
  return {
    ...
    tooltip: {
      ...
      extraCssText: "box-shadow: 0 4px 16px oklch(0% 0 0 / 0.25); border-radius: 8px;",
    },
    categoryAxis: {
      ...
      splitLine: {
        lineStyle: { color: border, opacity: 0.3, type: "dashed" as const },
      },
    },
    valueAxis: {
      axisLine: { show: false },
...
export const DEEPMAIL_ECHARTS_THEME = {
  ...
  legend: { textStyle: { color: "var(--color-muted, oklch(65% 0.01 280))", fontSize: 11 } },
  tooltip: {
  ...
  categoryAxis: {
    axisLine: { lineStyle: { color: "var(--color-border, oklch(26% 0.01 280))" } },
    axisTick: { show: false },
    axisLabel: { color: "var(--color-muted)", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" },
    splitLine: { lineStyle: { color: "var(--color-border, oklch(26% 0.01 280))", opacity: 0.3, type: "dashed" as const } },
  },
  valueAxis: {
    axisLine: { show: false },
    axisTick: { show: false },
    axisLabel: { color: "var(--color-muted)", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" },
    splitLine: { lineStyle: { color: "var(--color-border)", opacity: 0.3, type: "dashed" as const } },
  },
  grid: { left: 48, right: 16, top: 32, bottom: 32, containLabel: false },
  line: { smooth: true, lineStyle: { width: 2 }, symbolSize: 4, emphasis: { lineStyle: { width: 3 } } },
  bar: { barMaxWidth: 24, itemStyle: { borderRadius: [3, 3, 0, 0] } },
  pie: { borderWidth: 0, label: { color: "var(--color-muted)", fontSize: 11 } },
  radar: {
  ...
  gauge: {
    axisLine: { lineStyle: { width: 12, color: [[0.3, "#10b981"], [0.7, "#eab308"], [1, "#ef4444"]] } },
    axisTick: { show: false }, splitLine: { show: false }, axisLabel: { show: false },
    pointer: { width: 4 },
    title: { color: "var(--color-muted)", fontSize: 12 },
    detail: { color: "var(--color-fg)", fontSize: 28, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700 },
  },
} as const;
