...
interface IncidentReportCardLargeProps {
  isDarkMode?: boolean;
}

export const Component = ({ isDarkMode = false }: IncidentReportCardLargeProps) => {
  ...
  const gradientStopOpacity = isDarkMode ? 0.4 : 0.2;

  const criticalColorText = isDarkMode ? "text-[#F08083]" : "text-red-700";
  const criticalBgOpacity = isDarkMode ? "bg-[rgb(232,64,69)]/40" : "bg-red-100";
  
  const totalColorText = isDarkMode ? "text-[#40E5D1]" : "text-teal-700";
  const totalBgOpacity = isDarkMode ? "bg-[rgb(64,229,209)]/40" : "bg-teal-100";

  const metricIconFill = isDarkMode ? "#E84045" : "#EF4444";

  const getBadgeColors = (badgeType: "critical" | "improvement") => {
    if (badgeType === "critical") {
      return {
        rectFill: isDarkMode ? "rgb(232 64 69)" : "rgb(254 226 226)",
        rectOpacity: isDarkMode ? 0.4 : 1,
        arrowStroke: isDarkMode ? "#F08083" : "rgb(220 38 38)",
        arrowPath: "M9.50134 12.6111L14.0013 8.16663M14.0013 8.16663L18.5013 12.6111M14.0013 8.16663L14.0013 19.8333",
      };
    } else { // improvement
      return {
        rectFill: isDarkMode ? "rgb(64 229 209)" : "rgb(204 251 241)",
        rectOpacity: isDarkMode ? 0.4 : 1,
        arrowStroke: isDarkMode ? "#40E5D1" : "rgb(13 148 136)",
        arrowPath: "M18.4987 15.3889L13.9987 19.8334M13.9987 19.8334L9.49866 15.3889M13.9987 19.8334V8.16671",
      };
    }
  };
  ...
  return (
    <div
      className={cn(
        "flex flex-col justify-between pt-4 pb-4 rounded-3xl overflow-hidden w-[800px] h-[800px]",
    ...
    >
                  ...
                  gradient={
                    <Gradient
                      stops={[
                        ...
                        <GradientStop key="1" offset="100%" stopOpacity={gradientStopOpacity} />,
                      ]}
                    />
                  }
                />
      ...
      <div className="flex flex-col pl-8 pr-8 font-mono divide-y divide-gray-200 dark:divide-[#262631]">
        {metricItemsList.map((item) => {
          const badge = getBadgeColors(item.badgeType);
          return (
            <motion.div
              key={item.id}
            ...
            >
              <div className="flex flex-row gap-2 items-center text-base w-1/2 text-gray-600 dark:text-[#9A9AAF]">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path d={item.iconPath} fill={metricIconFill} />
                </svg>
              ...
              <div className="flex gap-2 w-1/2 justify-end items-center">
                ...
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect width="28" height="28" rx="14" 
                    fill={badge.rectFill} 
                    fillOpacity={badge.rectOpacity} 
                  />
                  <path 
                    d={badge.arrowPath} 
                    stroke={badge.arrowStroke} 
