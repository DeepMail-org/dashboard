...
MapLibreGL.config.apiKey = process.env.NEXT_PUBLIC_MAPTILER_API_KEY || "";
import {
	createContext,
	forwardRef,
	useCallback,
...
function MapControls({
	...
	const previousStateRef = useRef<{ bearing: number; pitch: number } | null>(null);
	...
	const handleResetBearing = useCallback(() => {
		...
		if (Math.abs(currentBearing) < 1 && Math.abs(currentPitch) < 1) {
			if (previousStateRef.current) {
				map.easeTo({
					bearing: previousStateRef.current.bearing,
					pitch: previousStateRef.current.pitch,
					duration: 300,
				});
				previousStateRef.current = null;
			}
		} else {
			previousStateRef.current = { bearing: currentBearing, pitch: currentPitch };
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
	...
	useEffect(() => {
		...
		const handleMouseMove = (e: MapLibreGL.MapLayerMouseEvent) => {
			const featureId = e.features?.[0]?.id as
				| string
				| number
				| undefined;
...
function Map3DBuildings({ enabled = true }: { enabled?: boolean }) {
	...
	useEffect(() => {
		...
		const sourceId = "maptiler_planet";
