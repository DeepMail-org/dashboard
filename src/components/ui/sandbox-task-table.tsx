...
import { SandboxTask, SandboxTaskStatus, useSandboxStore } from "@/stores/sandbox-store";
...
export function SandboxTaskTable({
  ...
  const handleStatusChange = (taskId: string, newStatus: SandboxTaskStatus) => {
    if (onStatusChange) {
  ...
  useEffect(() => {
    if (selectedTask) {
      const updatedTask = displayTasks.find(s => s.id === selectedTask.id);
      if (updatedTask) {
  ...
  const getRiskBars = (risk: number, status: SandboxTaskStatus) => {
    ...
    return (
      <div className="flex items-center gap-3">
        ...
        <span className={cn("text-xs font-mono font-medium min-w-[2rem]", 
          risk > 75 ? "text-danger" : risk > 40 ? "text-warning" : "text-success"
        )}>
          {risk || 0}
        </span>
      </div>
      ...
      case "running":
        return (
          <div className="px-3 py-1 rounded border border-accent/30 bg-accent/10 flex items-center justify-center">
            <span className="text-accent text-[11px] uppercase font-bold tracking-wider">Running</span>
          </div>
        );
      case "completed":
        return (
          <div className="px-3 py-1 rounded border border-success/30 bg-success/10 flex items-center justify-center">
            <span className="text-success text-[11px] uppercase font-bold tracking-wider">Completed</span>
          </div>
        );
      case "failed":
      case "cancelled":
        return (
          <div className="px-3 py-1 rounded border border-danger/30 bg-danger/10 flex items-center justify-center">
            <span className="text-danger text-[11px] uppercase font-bold tracking-wider">{status}</span>
          </div>
        );
      default:
        return (
          <div className="px-3 py-1 rounded border border-muted/30 bg-muted/10 flex items-center justify-center">
            <span className="text-muted-foreground text-[11px] uppercase font-bold tracking-wider">Pending</span>
          </div>
        );
    }
  };
  ...
  const getVerdictBadge = (verdict?: string) => {
    if (!verdict || verdict === "unknown") return <span className="text-muted">-</span>;
    switch (verdict) {
      case "malicious":
        return <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold border border-danger/30 text-danger bg-danger/10">Malicious</span>;
      case "suspicious":
        return <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold border border-warning/30 text-warning bg-warning/10">Suspicious</span>;
      case "clean":
        return <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold border border-success/30 text-success bg-success/10">Clean</span>;
      default:
  ...
  const getStatusGradient = (status: SandboxTaskStatus) => {
    switch (status) {
      case "running": return "from-accent/10 to-transparent";
      case "completed": return "from-success/5 to-transparent";
      case "failed": return "from-danger/10 to-transparent";
      default: return "from-transparent to-transparent";
    }
  };

  return (
    <div className={`flex flex-col min-h-0 overflow-hidden w-full relative bg-transparent ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-6 shrink-0 border-b border-border/10">
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm font-medium text-muted">
            {displayTasks.filter(s => s.status === "running").length} Running • {displayTasks.length} Total
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-6 pt-2">
        ...
        >
          {/* Headers */}
          <div className="grid grid-cols-12 gap-4 px-4 py-2 text-[10px] font-bold text-muted uppercase tracking-wider sticky top-0 bg-[#ebebeb] dark:bg-[#131315] border-b border-border z-10 shadow-xs">
                ...
                <div className="relative grid grid-cols-12 gap-4 items-center">
                  ...
                  <div className="col-span-3 flex items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-fg font-semibold text-[13px] truncate">{task.name}</span>
                      <span className="text-muted text-[10px] font-mono">{task.id}</span>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 flex flex-col justify-center">
                    <span className="text-fg text-[12px] font-medium">{task.worker || "Unassigned"}</span>
                    <span className="text-muted text-[10px]">{task.tenant}</span>
                  </div>
                  ...
                  {/* CPU / Risk */}
                  <div className="col-span-2">
                    {getRiskBars(task.risk || 0, task.status)}
                  </div>
            ...
            >
              ...
              <div className="relative bg-surface p-5 border-b border-border flex items-start justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-fg">{selectedTask.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="px-1.5 py-0.5 rounded bg-fg/10 text-secondary font-mono text-[10px]">{selectedTask.id}</span>
                      <span className="text-sm text-muted">{selectedTask.worker || "Unassigned"}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    <motion.button
                      ...
                      onClick={(e) => { e.stopPropagation(); closeTaskModal(); }}
              ...
              <div className="flex-1 flex flex-col min-h-0 bg-bg">
                 ...
                 <div className="flex-1 relative min-h-0 flex flex-col">
                    <SandboxTaskDetail task={selectedTask} activeTab={activeTab} />
                 </div>
              </div>
            </motion.div>
          )}
